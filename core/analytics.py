...
def tool_failure_report(limit: int = TOP_N) -> list:
    ...
    try:
        ...
        rows = []
    conn.close()
    return [{"tool": r[0], "failures_7d": r[1], "last": str(r[2]),
             "last_error": (r[3] or "")[:200]} for r in rows]
...
def learning_report() -> dict:
    ...
    try:
        ...
        cur.execute("SELECT user_input, old_agent, new_agent FROM routing_learnings "
...
def error_rate_report(hours: int = 24) -> dict:
    ...
    conn = _conn()
    cur = conn.cursor()
    out = {"requests": 0, "errors": 0, "error_rate": 0.0, "avg_latency": 0.0, "slowest": []}
    try:
        cur.execute(
        ...
        out.update({"requests": n, "errors": e,
...
def suggest_actions(report: dict) -> list:
    ...
    for t in report.get("failures", []):
        if t["failures_7d"] >= 5:
            tips.append(f"Tool '{t['tool']}' gagal {t['failures_7d']}x/7 hari — pertimbangkan karantina permanen/patch: {t['last_error'][:100]}")
...
def generate_report() -> dict:
    ...
    report = {"failures": failures, "learnings": learnings,
...
if __name__ == "__main__":
    ...
    print(f"Requests 24h: {rep['errors']['requests']} | errors: {rep['errors']['errors']} | avg latency: {rep['errors']['avg_latency']}s")
    print("\n-- Top failing tools --")
