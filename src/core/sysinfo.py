    ...
    try:
        if os.name == "nt":
            ...
            for letter in string.ascii_uppercase:
                ...
                parts.append(
                    {"mount": drive, "total_gb": _gb(u.total), "free_gb": _gb(u.free)}
                )
        else:
            seen = set()
            mounts = ["/"]
            ...
            for mp in mounts:
                ...
                try:
                    ...
                    continue
                parts.append(
                    {"mount": mp, "total_gb": _gb(u.total), "free_gb": _gb(u.free)}
                )
    except Exception:
        pass
    return parts
...
def format_sysinfo_block(info: dict) -> str:
    ...
    ram += (
        f" ({sticks} keping)"
        if sticks
        else (" (jml keping tak diketahui)" if info.get("ram_gb") else "")
    )
    parts = info.get("partitions") or []
    if parts:
        plist = ", ".join(
            f"{p['mount']} (bebas {p['free_gb']}/{p['total_gb']} GB)" for p in parts
        )
    ...
    if info.get("postgres_ok"):
        pg = f"Yes v{info.get('postgres_version') or '?'}"
    else:
        pg = "No (tak terhubung)"
    lines = [
        "## System",
