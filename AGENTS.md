...
## Run
- Install deps: `pip install -r requirements.txt` (venv di `venv/` — venv KANONIK tunggal; `.venv` sudah dihapus 2026-09). Dev tooling (ruff/mypy/pytest/bandit): `pip install -e . --group dev` (grup `[dependency-groups]` di `pyproject.toml`; `pip install -e ".[dev]"` TIDAK bekerja — bukan `[project.optional-dependencies]`).
...
- `auth.py` — facade re-export seluruh simbol publik submodul auth (API keys + auth web).
- `account.py` — akun manusia: `register_with_password` (opsional `username` — auto dari local-part, unik via `ix_users_username_lower`; email duplikat → exception `EmailAlreadyRegistered` → route 409 + `hint_provider` = provider OAuth akun lama), `authenticate_password` (identifier = email ATAU username, case-insensitive), verifikasi email (`resend_verification`/`confirm_email_verification`), reset password (`issue_password_reset`/`confirm_password_reset`), siklus 2FA (`issue_2fa_challenge`/`resolve_2fa_challenge`/`enable`/`confirm`/`disable_totp`), `link_oauth_identity` (deteksi urut: by_provider → auto-link by email → buat baru; **fail-closed**: profil OAuth dengan `email_verified` false/absen ditolak `email_unverified` sebelum auto-link/buat baru).
- `auth_db.py` — helper DB auth: `_ensure_auth_columns()` (ALTER TABLE lazy kolom auth di `users` — tabel tidak dikelola Alembic) + query user/sesi/token.
- `emailer.py` — kirim email verifikasi/reset via aiosmtplib (`SMTP_*` env); `email_enabled()`; gagal kirim saat register → 503 (fail-closed).
- `oauth.py` — `start_oauth` / `exchange_oauth` (Google & GitHub): state `auth_oauth_states` TTL (`AUTH_OAUTH_STATE_TTL_MIN`, default 10), redirect `/?auth=...` untuk error.
- `passwords.py` — hash Argon2id (`argon2-cffi`) + `password_policy_ok` (min `AUTH_MIN_PASSWORD_LEN`, default 8).
- `sessions.py` — sesi cookie web: `ayesh_session` (HttpOnly, SameSite=Lax, Secure bila `AUTH_COOKIE_SECURE=1`, TTL `AUTH_SESSION_TTL_HOURS` default 168) + CSRF double-submit `ayesh_csrf` (dibaca JS → header `X-CSRF-Token`); `create_session`/`verify_session`/`verify_csrf`/`revoke_*`.
- `totp.py` — TOTP pyotp (secret/URI/verify) + 10 backup code (sekali pakai, hash disimpan).
- `auth_keys.py` — API key multi-user (tetap): `users` table, sha256, `X-API-Key` header, `REQUIRE_API_KEY=1`. ContextVar `current_user`; prefs/proyek scoped per user (composite PK). Endpoints: `POST/GET /users`, `DELETE /users/{id}`, `POST /users/{id}/rotate` (rotasi API key; key lama tetap valid `API_KEY_ROTATION_GRACE_HOURS` jam, default 24). Kolom rotasi (`old_key_hash`/`old_prefix`/`old_key_expires_at`) dimigrasi lazy via `_ensure_rotation_columns()` (ALTER TABLE, sekali per proses) karena tabel `users` tidak dikelola Alembic.
...
- `routes_billing.py` — `GET /billing/history`, `GET /billing/history/{external_ref}`, `GET /billing/status/{external_ref}`.
- `routes_webhooks.py` — `POST /webhooks/vip-upgrade` (dari server billing).
...
- **Auth web (akun manusia)**: sesi web = cookie `ayesh_session` (HttpOnly) + CSRF double-submit `ayesh_csrf` → header `X-CSRF-Token` (semua mutasi cookie-based wajib; API-key flow `X-API-Key` tetap berlaku tanpa CSRF). Email wajib diverifikasi sebelum login (`AUTH_REQUIRE_EMAIL_VERIFICATION=1`); kirim email gagal saat register → **503 fail-closed** (akun bisa dibuat tapi belum aktif; `AUTH_DEV_VERIFY=1` hanya dev). Register/login memakai email ATAU `username` (`AuthLoginRequest.identifier`; kolom `email` lama tetap diterima, wajib salah satu terisi); duplikat email saat register → **409** + `hint_provider` (`google`/`github`/`null`), metric outcome `conflict`. `GET /auth/me` mengekspos `username` + `connected_providers`. 2FA TOTP opsional (backup code sekali pakai). OAuth Google/GitHub via `/auth/oauth/{provider}` (state TTL 10 menit); callback gagal → redirect `/?auth=error&reason=` — `email_unverified` (gate `email_verified` false/absen, disurface) selain itu generik `oauth_failed`. Rate-limit scope khusus: `register` (burst 2, sustained 5 per menit, IP-only — path `/users/register`) dan `auth` (burst 5, sustained 20 — prefix `/auth`), di luar scope default. Telemetri: counter `ayesh_auth_events_total{event,outcome}` (register/login/2fa/reset/oauth, sukses/gagal/conflict). Env: `AUTH_*`/`SMTP_*`/`AUTH_GOOGLE_CLIENT_ID`/`AUTH_GITHUB_CLIENT_ID` — lihat `.env.example`.
