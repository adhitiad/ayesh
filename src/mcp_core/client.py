...
class MCPClientManager:
    ...
    def __init__(self, config_path="mcp_core/mcp.json"):
    ...
    async def _connect_server(self, server_name: str) -> Optional[ClientSession]:
        ...
        try:
            ...
            read, write = await asyncio.wait_for(
                stdio_client(params).__aenter__(), timeout=30
            )
            session = await asyncio.wait_for(
                ClientSession(read, write).__aenter__(), timeout=30
            )
            ...
            if "TaskGroup" in error_msg:
                ...
                logger.warning(
                    f"Gagal menghubungkan ke {server_name}: {error_msg[:100]}"
                )
            return None
    ...
    async def list_all_tools(self) -> List[Dict[str, Any]]:
        ...
        for server_name, server_config in servers.items():
            ...
            if not is_stdio and not is_remote:
                logger.debug(
                    f"Melewati server {server_name}: bukan stdio maupun remote MCP server"
                )
                continue
            ...
            try:
                if is_stdio:
                    ...
                    async with stdio_client(params) as (read, write):
                        async with ClientSession(read, write) as session:
                            ...
                            logger.info(
                                f"Berhasil terhubung ke {server_name}: {len(result.tools)} tools tersedia"
                            )
                elif is_remote:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Streamable HTTP gagal untuk {server_name}: {str(e_http)[:200]}"
                        )
                        # Fallback ke SSE
                        try:
                            async with sse_client(url=url, headers=headers or None) as (
                                read,
                                write,
                            ):
                                async with ClientSession(read, write) as session:
                            ...
                            logger.debug(
                                f"Remote server {server_name} tidak tersedia via SSE: {str(e_sse)[:200]}"
                            )
            except asyncio.TimeoutError:
                ...
                if is_remote:
                    logger.debug(
                        f"Remote server {server_name} tidak tersedia: {error_msg[:120]}"
                    )
                else:
                    logger.warning(
                        f"Gagal mengambil tools dari {server_name}: {error_msg[:120]}"
                    )

        if not all_tools:
            logger.info(
                "Tidak ada remote MCP tools yang berhasil dimuat. Menggunakan tools lokal saja."
            )
    ...
    async def call_tool(
        ...
        try:
            ...
            if is_stdio:
                ...
                return (
                    f"Error: Server {server_name} tidak mendukung stdio maupun remote"
                )
        except asyncio.TimeoutError:
