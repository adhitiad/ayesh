...
_last_usage: ContextVar[dict] = ContextVar("llm_usage", default={})
...
def _write_stats(session_id: str, result: dict, latency: float, success: bool, usage: dict):
    try:
        ...
        cur.execute(
            ...
            (session_id, result.get("agent_type", ""), tools[:500], round(latency, 2),
             usage.get("prompt_tokens", 0), usage.get("completion_tokens", 0),
             usage.get("total_tokens", 0), success),
        )
        conn.close()
    except Exception:
        pass
...
def record_usage(fn):
    ...
    def wrapper(user_input: str, session_id: str, *args, **kwargs):
        ...
        try:
            ...
            raise
        _write_stats(session_id, result if isinstance(result, dict) else {},
                     time.time() - t_start, True, pop_usage())
        return result
    return wrapper
...
def summarize_usage(hours: int = 24) -> dict:
    ...
    by_agent = [{"agent": r[0], "count": r[1], "avg_latency": round(float(r[2] or 0), 2), "tokens": int(r[3] or 0)} for r in cur.fetchall()]
    conn.close()
    return {"hours": hours, "requests": int(n or 0),
            "success": int(okc or 0), "avg_latency_s": round(float(avg_lat or 0), 2),
            "total_tokens": int(tot or 0), "prompt_tokens": int(prm or 0),
            "completion_tokens": int(cmp_ or 0), "by_agent": by_agent}
