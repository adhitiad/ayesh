...
## Linting
- Run `ruff check .` before committing (config in `.ruff.toml`). All 298 tests must pass: `python -m unittest discover -s tests`.
...
- `auth.py` — multi-user API keys (`users` table, sha256, `X-API-Key` header, `REQUIRE_API_KEY=1`). ContextVar `current_user`; prefs/proyek scoped per user (composite PK). Endpoints: `POST/GET /users`, `DELETE /users/{id}`.
...
- `logger.py` — standard `logging` setup with API key redaction.
...
- `usage.py` — usage tracking decorator + token via ContextVar; tabel `request_stats`; `GET /usage/summary`, `GET /usage/recent`.
...
- `routes_agents.py` — `POST/GET /users`, `DELETE /users/{id}`, `GET /agents`.
...
- `routes_system.py` — health, metrics, analytics, usage, audit, config, keyword CRUD.
...
- `src/plugins/core_tools.py` — real tool definitions: `tulis_kode`, `baca_file`, `cari_web`, `baca_url`, `jalankan_python`, `panggil_mcp`, `learn_keyword`, `get_current_time`, `minta_review`, `ingat_preferensi`/`lihat_preferensi`, `simpan_proyek`/`catat_proyek`/`lihat_proyek`, `info_sistem`, `set_target_dir`.
...
- `tests/` — 298 tests (unittest, fast, no LLM/infra) + end-to-end eval + security regression.
...
- **Rate limit**: `route_request()` sleeps 3s per call under a lock. Expect slow sequential runs. Rate limiter lives in `src/core/system/rate_limit.py`.
