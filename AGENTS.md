...
## Linting
- Run `ruff check .` before committing (config in `.ruff.toml`). All 684 tests must pass: `python -m unittest discover -s tests` (atau `venv\Scripts\python.exe -m unittest ...`).
...
- `account.py` — akun manusia: `register_with_password`, `authenticate_password`, verifikasi email (`resend_verification`/`confirm_email_verification`), reset password (`issue_password_reset`/`confirm_password_reset`), siklus 2FA (`issue_2fa_challenge`/`resolve_2fa_challenge`/`enable`/`confirm`/`disable_totp`), `link_oauth_identity` (auto-link by email; **fail-closed**: email OAuth belum terverifikasi → tolak, jangan set `email_verified`).
...
- `tests/` — 684 tests (unittest, fast, no LLM/infra) + end-to-end eval + security regression.
...
- **Auth web (akun manusia)**: sesi web = cookie `ayesh_session` (HttpOnly) + CSRF double-submit `ayesh_csrf` → header `X-CSRF-Token` (semua mutasi cookie-based wajib; API-key flow `X-API-Key` tetap berlaku tanpa CSRF). Email wajib diverifikasi sebelum login (`AUTH_REQUIRE_EMAIL_VERIFICATION=1`); kirim email gagal saat register → **503 fail-closed** (akun bisa dibuat tapi belum aktif; `AUTH_DEV_VERIFY=1` hanya dev). 2FA TOTP opsional (backup code sekali pakai). OAuth Google/GitHub via `/auth/oauth/{provider}` (state TTL 10 menit). Rate-limit scope khusus: `register` (burst 2, sustained 5 per menit, IP-only — path `/users/register`) dan `auth` (burst 5, sustained 20 — prefix `/auth`), di luar scope default. Telemetri: counter `ayesh_auth_events_total{event,outcome}` (register/login/2fa/reset/oauth, sukses/gagal). Env: `AUTH_*`/`SMTP_*`/`AUTH_GOOGLE_CLIENT_ID`/`AUTH_GITHUB_CLIENT_ID` — lihat `.env.example`.
