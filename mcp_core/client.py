...
class MCPClientManager:
    ...
    def _check_server_package(self, package_name: str) -> bool:
        """Cek apakah package MCP server sudah terinstall."""
        try:
            result = subprocess.run(
    ...
    async def _connect_server(self, server_name: str) -> Optional[ClientSession]:
        ...
        try:
            ...
            session = await asyncio.wait_for(
                ClientSession(read, write).__aenter__(),
                timeout=30
            )
            ...
            if "TaskGroup" in error_msg:
                logger.warning(f"Server {server_name} gagal inisialisasi (kemungkinan kompatibilitas Windows): {error_msg[:100]}")
            else:
        ...
        for server_name, server_config in servers.items():
            ...
            try:
                if is_stdio:
                    ...
                    logger.debug(f"Mencoba remote server {server_name} URL={url} headers_keys={list(headers.keys()) if headers else []}")
                    # Coba Streamable HTTP dulu, fallback ke SSE
                    try:
                        async with streamablehttp_client(url=url, headers=headers or None) as (read, write):
                            async with ClientSession(read, write) as session:
                                await session.initialize()
                                result = await session.list_tools()
                                for tool in result.tools:
                                    ...
                                    all_tools.append(tool_data)
                                self._connected_servers[server_name] = True
                                logger.info(f"Berhasil terhubung ke remote {server_name} via Streamable HTTP: {len(result.tools)} tools tersedia")
                    except Exception as e_http:
                        logger.debug(f"Streamable HTTP gagal untuk {server_name}: {str(e_http)[:200]}")
                        # Fallback ke SSE
                        try:
                            async with sse_client(url=url, headers=headers or None) as (read, write):
                                async with ClientSession(read, write) as session:
                                    await session.initialize()
                                    result = await session.list_tools()
                                    for tool in result.tools:
                                        ...
                                        all_tools.append(tool_data)
                                    self._connected_servers[server_name] = True
                                    logger.info(f"Berhasil terhubung ke remote {server_name} via SSE: {len(result.tools)} tools tersedia")
