...
from core.models import Monologue
...
def get_monologues(user_id: str, agent_type: str):
    """Ambil semua monologue untuk user dan agent tertentu."""
    SessionLocal = _session_local()
    with SessionLocal() as session:
        results = session.execute(
            select(Monologue)
            .where(Monologue.user_id == user_id, Monologue.agent_type == agent_type)
            .order_by(Monologue.timestamp)
        ).scalars().all()
        return results
