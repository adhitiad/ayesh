...
STOPWORDS = frozenset(
    """
yang dan di ke dari untuk dengan pada adalah ini itu sebagai dalam oleh karena
atau juga tidak ada akan telah sudah bisa dapat harus agar supaya jika kalau
the and or of to in on for with as by from at an a is are was were be been
yang saya kamu anda dia mereka kita kami nya lah kah pun tak jangan
""".split()
)
...
def _tokens(text: str) -> set:
    return {
        t for t in _TOKEN_RE.findall(text.lower()) if len(t) > 2 and t not in STOPWORDS
    }
...
def retrieve(
    query: str, top_k: int = 3, min_overlap: int = 2, root: Path | None = None
) -> list:
